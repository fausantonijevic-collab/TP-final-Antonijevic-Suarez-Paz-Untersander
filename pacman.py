...
class pacman:
    ...
    def dibujar(self, superficie, offset_y=0):
        ...
        angulo_base_rad = math.radians(angulo_base[self.direccion])
        apertura_rad    = math.radians(self.apertura)

        puntos = [(cx, cy)]
        pasos  = 12
        for i in range(pasos + 1):
            t = i / pasos
            angulo = angulo_base_rad - apertura_rad + t * 2 * apertura_rad
            bx = cx + self.radio * math.cos(angulo)
            by = cy - self.radio * math.sin(angulo)
            puntos.append((bx, by))
